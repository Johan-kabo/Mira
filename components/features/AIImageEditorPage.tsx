...
const AIImageEditorPage: React.FC = () => {
        ...
        <div className="max-w-7xl mx-auto">
            <div className="text-center mb-12">
                ...
                <p className="text-gray-400 max-w-2xl mx-auto mt-4">
                    Upload one or more images, then describe how you want to combine or edit them. For example, you can merge concepts or transfer artistic styles.
                </p>
            </div>
            ...
            <div className="grid lg:grid-cols-2 gap-8 items-start">
                ...
                <div className="bg-[#1c162d]/50 border border-white/10 rounded-2xl p-6 sm:p-8 space-y-6">
                    <div>
                     ...
                     {originalImages.length > 0 && (
                        <div className="space-y-2">
                            <h3 className="text-lg font-medium text-white">Image Preview{originalImages.length > 1 ? 's' : ''}</h3>
                     ...
                     <div>
                        <label htmlFor="prompt" className="block text-lg font-medium text-white mb-2">2. Décrivez votre modification</label>
                        <textarea id="prompt" value={prompt} onChange={(e) => setPrompt(e.target.value)} rows={3}
                            ...
                            placeholder="ex: 'Mélange ces deux images' ou 'Applique le style de la deuxième image à la première'"></textarea>
                    </div>
                    ...
                    <button onClick={handleEditImage} disabled={loading || imageFiles.length === 0 || !prompt} className="w-full bg-gradient-to-r from-purple-600 to-indigo-600 text-white px-6 py-3 rounded-full flex items-center justify-center space-x-2 hover:from-purple-700 hover:to-indigo-700 transition-all duration-300 disabled:opacity-50 disabled:cursor-not-allowed">
                        {loading ? 'Édition en cours...' : 'Appliquer'}
                    </button>
                </div>
                ...
                <div className="bg-[#1c162d]/50 border border-white/10 rounded-2xl p-6 sm:p-8 min-h-[400px] flex flex-col justify-center items-center">
                    ...
                    {!loading && !error && !resultImage && (
                        <p className="text-gray-500">Téléchargez une image et décrivez votre modification.</p>
                    )}
                    {resultImage && (
                        <div className="w-full text-center">
                            ...
                            <a href={resultImage} download={`edited-image-${Date.now()}.png`} className="mt-6 inline-block bg-white text-black px-6 py-2 rounded-lg hover:bg-gray-200 transition-colors">Télécharger</a>
                        </div>
                    )}
                </div>
            </div>
